...
namespace AbstractSecurityShopServiceImplementDataBase.Implementation
{
    public class ReptServiceDB : IReptService
    {
        ...
        public ReptServiceDB(AbstractSecurityShopDbContext context)
        {
        ...
        public void SaveTechnicsPrice(ReptBindingModel model)
        {
            ...
            try
            {
                ...
                var technics = context.Technics.ToList();
                //создаем таблицу
                var paragraphTable = document.Paragraphs.Add(Type.Missing);
                var rangeTable = paragraphTable.Range;
                var table = document.Tables.Add(rangeTable, technics.Count, 2, ref missing, ref missing);
                ...
                for (int i = 0; i < technics.Count; ++i)
                {
                    table.Cell(i + 1, 1).Range.Text = technics[i].TechnicsName;
                    table.Cell(i + 1, 2).Range.Text = technics[i].Price.ToString();
                }
        ...
        {
            return context.Storages.ToList().GroupJoin(context.StorageEquipments.Include(r => r.Equipment).ToList(), stock => stock, stockComponent => stockComponent.Storage, (stock, stockCompList) => new StorageLoadViewModel
            {
                StorageName = stock.StorageName,
                TotalCount = stockCompList.Sum(r => r.Count),
                Equipments = stockCompList.Select(r => new Tuple<string,
               int>(r.Equipment.EquipmentName, r.Count))
            }).ToList();
        }
        ...
        {
            return context.Orders.Include(rec => rec.Customer).Include(rec => rec.Technics).Where(rec => rec.DateCreate >= model.DateFrom && rec.DateCreate <= model.DateTo).Select(rec => new CustomerOrdersViewModel
            {
        ...
        public void SaveCustomerOrders(ReptBindingModel model)
        {
           ...
           iTextSharp.text.Font.BOLD);
            table.AddCell(new PdfPCell(new Phrase("ФИО заказчика", fontForCellBold))
            {
            ...
            table.AddCell(new PdfPCell(new Phrase("Техника", fontForCellBold))
            {
                HorizontalAlignment = Element.ALIGN_CENTER
            });
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                table.AddCell(cell);
                cell = new PdfPCell(new Phrase(list[i].TechnicsName, fontForCells));
                table.AddCell(cell);
