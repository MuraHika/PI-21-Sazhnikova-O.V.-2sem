...
namespace AbstractSecurityShopView
{
    public static class MailCustomer
    {
        ...
        public static void Connect()
        {
            ...
            response = SendRequest(reader, writer, string.Format("USER {0}", ConfigurationManager.AppSettings["MailLogin"]), "Ошибка авторизации, неверный логин");
            response = SendRequest(reader, writer, string.Format("PASS {0}", ConfigurationManager.AppSettings["MailPassword"]), "Ошибка авторизации, неверный пароль");
            CheckMail();
        }
        ...
        private static void GetMessages(int number)
        {
            ...
            while (true)
            {
                ...
                if (response.Length > 4)
                {
                    ...
                    if (!string.IsNullOrEmpty(messageId) && !string.IsNullOrEmpty(from) && !string.IsNullOrEmpty(orderSubjectMessage) && !string.IsNullOrEmpty(date))
                    {
                        APICustomer.PostRequest<MessageInfoBindingModel, bool>("api/MessageInfo/AddElement", new MessageInfoBindingModel
                        {
                            MessageId = messageId,
